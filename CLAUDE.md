...
4. **Negotiation Window Score** (0–100) ανά περίοδο: 50% συμμετρία ισχύος + 30%
   αμοιβαία πτωτική τάση + 20% κοινωνική **σταθερότητα** (όχι αστάθεια/πίεση --
   κατεύθυνση διορθώθηκε 2026-08-21, βλ. SEED_SOURCE.md §10: η διπλωματική
   δείχνει ότι η αστάθεια αυξάνει το πολιτικό κόστος υποχώρησης, δυσκολεύει
   τη συμφωνία). Ντετερμινιστικό, testable.
...
- **Ύφος .md αρχείων (ΜΟΝΙΜΟΣ κανόνας, από 2026-08-21):** ό,τι γράφεται σε
  οποιοδήποτε .md αρχείο του project (README.md, SEED_SOURCE.md,
  PROJECT_STATUS.md, CLAUDE.md, κ.λπ.) είναι σε ΠΡΩΤΟ ΠΡΟΣΩΠΟ — σαν να
  μιλάει ο δημιουργός του project, περιγραφικό ύφος, ΟΧΙ οδηγία προς τρίτο
  πρόσωπο. ΛΑΘΟΣ: "Χρησιμοποίησε GDP_absolute_usd σε λογαριθμική κλίμακα
  επειδή..." ΣΩΣΤΟ: "Χρησιμοποιώ το GDP_absolute_usd σε λογαριθμική κλίμακα
  επειδή..." ή "Το GDP_absolute_usd χρησιμοποιεί λογαριθμική κλίμακα
  επειδή...". Ισχύει για ΝΕΟ περιεχόμενο από εδώ και πέρα — δεν χρειάζεται
  αναδρομική διόρθωση ήδη σωστού υλικού (π.χ. README.md "About this
  project"/"Beyond this case study", SEED_SOURCE.md §9/§10 είναι ήδη σε
  αυτό το ύφος), μόνο σημείωση αν εντοπιστεί λάθος ύφος στην πορεία.
