...
//
// Απαιτεί login (backend: Depends(get_current_user) στο POST /compare,
// ΟΧΙ require_admin -- οποιοσδήποτε συνδεδεμένος χρήστης), ίδιο μοτίβο
// με το SynthesisPage.
export function ComparePage() {
  ...
  const { data: events, isLoading, error } = useQuery({
    ...
    // Δεν έχει νόημα να φέρουμε τη λίστα events για το picker αν ο
    // επισκέπτης δεν είναι καν συνδεδεμένος -- θα δει μόνο το notice.
  ...
  return (
    <div>
      ...
      {!isAuthenticated && <LoginRequiredNotice toolName="Compare" />}
      {isAuthenticated && isLoading && <LoadingState label="Loading events…" />}
      {isAuthenticated && error && <ErrorState error={error} />}
