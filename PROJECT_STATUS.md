...
## Μεθοδολογική αναθεώρηση Power Index -- ECONOMIC/MILITARY (2026-08-21)

**Αφορμή:** ερώτηση επιβεβαίωσης "ποιοι πραγματικά είναι οι
indicator_types ανά κατηγορία" αποκάλυψε δύο προβλήματα: (1) MILITARY
συνέκρινε Serbia `military_expenditure_pct_gdp` (δικό της spending) με
Kosovo `troop_presence_index` (researcher estimate ΞΕΝΗΣ στρατιωτικής
παρουσίας) -- εννοιολογικά ασύμβατα κάτω από την ίδια κατηγορία· (2)
`unemployment_rate` είχε ανάποδη κατεύθυνση στο `normalize()` (υψηλότερη
ανεργία -> υψηλότερο economic score, confirmed με πραγματικά νούμερα:
Serbia 2005 ανεργία 20.85% έπαιρνε ΥΨΗΛΟΤΕΡΟ score από το 2023, 8.27%).

**Πλήρες αποτέλεσμα, τελική έκδοση (βλ. SEED_SOURCE.md §3.5/3.6 για το
πλήρες, βήμα-βήμα change-log δύο γύρων -- εδώ μόνο η τελική κατάσταση):**

- **ECONOMIC** = `GDP_growth` + `GDP_absolute_usd` (λογαριθμική κλίμακα,
  $1B-$100B, ελέγχθηκε ευσταθές με εναλλακτικό $500M-$200B) +
  `unemployment_rate` (διορθωμένη κατεύθυνση), ισοβαρή μέσο όρο όπου
  διαθέσιμα. Ενδιάμεσα δοκιμάστηκε *αντικατάσταση* GDP_growth->
  GDP_absolute_usd (όχι συνδυασμός) -- έχασε την ανίχνευση του
  οικονομικού σοκ Σερβίας 1999 (P1), διορθώθηκε με συνδυασμό.
- **MILITARY** = Kosovo `military_expenditure_pct_gdp` (ΙΔΙΟ indicator/
  πηγή με Serbia, World Bank/SIPRI `MS.MIL.XPND.GD.ZS`, verified) αντί
  για `troop_presence_index`. 2008/2013/2023 πραγματικά reported·
  1999/2005/2007 = 0.0, **τεκμηριωμένο ιστορικό γεγονός** (KSF ιδρύθηκε
  Ιαν 2009, το προγενέστερο KPC ήταν ρητά μη-στρατιωτικό), ΟΧΙ εκτίμηση
  κενού. `troop_presence_index` παραμένει context-only.
- Εναλλακτικά για MILITARY που ερευνήθηκαν και απορρίφθηκαν:
  `active_personnel` (World Bank: 0/25 έτη Kosovo δεδομένα· δευτερεύουσες
  πηγές ασύμφωνες, καλύπτουν αξιόπιστα μόνο 1 έτος), Global Firepower
  Index (ιδιωτική, αδιαφανής μεθοδολογία).

**P1-P5 validation tests (`test_validation_targets.py`) -- τελικό
αποτέλεσμα μετά την πλήρη αναθεώρηση:**

| # | Πρόταση | Τελικό αποτέλεσμα |
|---|---|---|
| P1 | Σερβία καταρρέει 1999 | ✅ επιβεβαιώνεται, **επαληθευμένο like-for-like** (1998=80.82 vs 1999=58.46 full-set, 54.7 like-for-like -- κατεύθυνση robust και στις δύο μεθόδους) |
| P2 | Ανάκαμψη μετά το 2000 | ✅ 2005=51.79 -> 2007=54.5 |
| P3 | 2013 στιγμή ωρίμανσης | ✅ 2013=56.75 vs 2023=56.13 (margin 0.62) -- **ελέγχθηκε ευστάθεια** με εναλλακτικό log-scale εύρος, 2013 κερδίζει και εκεί (57.52 vs 56.64, margin 0.88 -- robust) |
| P4 | Power Gap στενεύει 2013->2023 | ✅ 14.61 -> 11.53 (πέρασε από ενδιάμεση ανατροπή -- γραμμική κλίμακα το διεύρυνε, log-scale το διόρθωσε) |
| P5 | Κόσοβο χωρίς ανεξάρτητη BATNA | 🔄 ΜΕΡΙΚΩΣ επιβεβαιώνεται τώρα (πριν: καθόλου) -- military σταθερά χαμηλό (0-15.92) σε όλα τα έτη, economic μεικτό (χαμηλό 2005/07, μέτριο 2013/23) |

**Ενδιάμεσα ευρήματα που αξίζει να θυμόμαστε** (πλήρες αφήγημα στο
SEED_SOURCE.md): γραμμική κλίμακα GDP_absolute_usd έκανε το Kosovo
economic component σχεδόν σταθερό χαμηλό (5-10.5%) -- "δομικό ταβάνι"
ανεξάρτητο από πραγματική δυναμική, προκάλεσε reversal στο P3 ΚΑΙ στο
P4 ταυτόχρονα. Log-scale το διόρθωσε και τα δύο. Το κενό Kosovo military
πριν το 2008 έσπαγε ξεχωριστά το `previous_year` lookup του P3 (μηδένιζε
το trend_score, 30% βάρος) -- ανεξάρτητο πρόβλημα από το scale, διορθώθηκε
με το ιστορικό 0.0 fill.

`pytest -q`: **89 passed** (ήταν 87 πριν τη μεθοδολογική αναθεώρηση --
+2 νέα unit tests για το `unemployment_rate` direction fix). `seed.py`:
74 indicator εγγραφές (ήταν 71 πριν το Kosovo pre-2009 military fill).

**Ακόμα δεν έγινε (backlog, ενημερωμένο 2026-08-21):**
- ✅ Docker Compose πλήρες stack -- έγινε νωρίτερα σε αυτό το session
  (api+frontend services, πλήρες `down -v && up --build` test με
  browser verification)
- 🔶 README.md -- σε εξέλιξη (About this project section + τεχνικό
  μέρος γραμμένα), κατάσταση commit άγνωστη προς το παρόν -- ο χρήστης
  διαχειρίζεται το git χειροκίνητα από εδώ και πέρα, βλ. σημείωση
  παρακάτω
- ❌ Polish pass (γενική επισκόπηση/καθάρισμα πριν το τέλος)

**ΣΗΜΕΙΩΣΗ workflow (2026-08-21):** ο χρήστης ζήτησε ρητά να μην τρέξω
ΚΑΝΕΝΑ git command (add/commit/push, ούτε καν read-only log/status) --
το git το χειρίζεται ο ίδιος χειροκίνητα από εδώ και πέρα. Οι αλλαγές
κώδικα/τεκμηρίωσης αυτού του session (analytics.py, seed.py,
test_validation_targets.py, SEED_SOURCE.md, PROJECT_STATUS.md) είναι
γραμμένες στο δίσκο αλλά ΔΕΝ committed από εμένα.
